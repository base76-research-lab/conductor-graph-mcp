...
GET_AGENT_GRAPH_TOOL = Tool(
    ...
    description=(
        "Returns the full CognOS agent graph as a structured dict. "
        "Call this to get an immediate system overview: all nodes (gateway, agents, "
        "memory, bus) and edges (communication flow) with live status."
    ),
...
GET_NODE_STATUS_TOOL = Tool(
    ...
    description="Returns live status and metrics for a specific node in the CognOS graph.",
    inputSchema={
        "type": "object",
        "properties": {
            "node_id": {
                "type": "string",
                "description": (
                    "Node ID, e.g. 'trustplane-gateway', 'agent-critic', "
...
GET_EDGES_TOOL = Tool(
    name="get_edges",
    description="Returns only the edges (communication flow) in the CognOS graph.",
    inputSchema={"type": "object", "properties": {}},
)
...
GET_BLOCKED_NODES_TOOL = Tool(
    ...
    description=(
        "Returns nodes with status 'blocked' or 'error'. "
        "Quick shortcut to see what is broken or unreachable right now."
    ),
    inputSchema={"type": "object", "properties": {}},
)
...
async def call_tool(name: str, arguments: dict[str, Any]) -> CallToolResult:
    ...
    if name == "get_agent_graph":
        ...
        return CallToolResult(
            content=[TextContent(
                type="text",
                text=json.dumps({"error": f"Node '{node_id}' not found.", "known_nodes": known}),
            )],
            is_error=True,
        )
                ...
                text=json.dumps(
                    {
                        ...
                        "message":       "No blocked nodes." if not blocked
                                         else f"{len(blocked)} node(s) with issues.",
                    },
                    indent=2,
                ),
            )],
